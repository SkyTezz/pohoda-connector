...
const invoiceArgs = {
  ...
  idempotencyKey: "order-invoice:2026002987:r1",
  reason: "daily close",
};
...
describe("approval write gate", () => {
  it("agent proposes; nothing is sent; ids are deterministic", async () => {
    ...
    expect(result.datapackId).toBe("TEST-order-invoice:2026002987:r1");
    expect(h.client.sent).toHaveLength(0);

    const proposal = await h.outbox.get(result.proposalId as number);
    expect(proposal?.xml).toContain('id="TEST-order-invoice:2026002987:r1"');
    expect(proposal?.xml).toContain('<dat:dataPackItem id="order-invoice:2026002987:r1"');
    expect(proposal?.xml).toContain("<typ:ids>order-invoice:2026002987:r1</typ:ids><typ:exSystemName>TEST</typ:exSystemName>");
    expect(proposal?.xml).toContain('<typ:numberRequested checkDuplicity="true">2026002987</typ:numberRequested>');
  ...
  it("send transmits the frozen XML with duplicity check and records POHODA's id", async () => {
    ...
    h.client.queue.push(okResponse("order-invoice:2026002987:r1", 4711));
    ...
    expect(h.client.sent[0].options?.instance).toBe("TEST-order-invoice:2026002987:r1");
  ...
  it("auto-send on approve sends immediately", async () => {
    ...
    const { proposalId } = parseToolJson(await h.registryFor(AGENT).call("pohoda_create_invoice", invoiceArgs));
    h.client.queue.push(okResponse("order-invoice:2026002987:r1"));
  ...
  it("transport failure → failed, replay re-sends identical XML; POHODA duplicate → sent", async () => {
    ...
    h.client.queue.push(errorResponse("order-invoice:2026002987:r1", "Duplicitní doklad: dataPackItem id již byl importován"));
  ...
  it("POHODA refusal → refused with the note kept", async () => {
    ...
    const { proposalId } = parseToolJson(await h.registryFor(AGENT).call("pohoda_create_invoice", invoiceArgs));
    await h.registryFor(HUMAN).call("pohoda_proposal_approve", { id: proposalId });
    h.client.queue.push(errorResponse("order-invoice:2026002987:r1", "Členění DPH UDobch neexistuje"));
  ...
  it("direct mode sends immediately with deterministic ids", async () => {
    ...
    h.client.queue.push(okResponse("order-invoice:2026002987:r1", 99));
    ...
    expect(h.client.sent[0].xml).toContain('id="TEST-order-invoice:2026002987:r1"');
  });
});
